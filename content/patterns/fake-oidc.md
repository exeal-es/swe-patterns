...
Una pieza de infra propia — `fake-oidc` — que implementa el subconjunto de OIDC que una API cliente necesita para autenticar (`/authorize`, `/token`, `/.well-known/jwks.json`, `/oidc/v1/userinfo`) y nada más: sin registro de usuarios, sin gestión de contraseñas, sin flujos que nadie usa.

Se configura por variables de entorno con una lista fija de clientes (`OIDC_CLIENTS`, con `clientId` y `redirectUris`) y una lista fija de usuarios hardcodeados (`OIDC_USERS`, cada uno con `sub`, `email`, `name` y `roles`) — sin base de datos ni persistencia entre reinicios. Ejemplo de configuración en `docker-compose.yaml`:
...
fake-oidc:
  ...
  environment:
    ...
    OIDC_CLIENTS: >-
      [{"clientId":"mi-app","redirectUris":["http://localhost:3000/callback","http://localhost:5173/callback"],"postLogoutRedirectUris":[...]},
      {"clientId":"mi-app-admin","redirectUris":["http://localhost:3001/callback"]}]
    OIDC_USERS: >-
...
Se distribuye como imagen de contenedor (`ghcr.io/ne2-studio/fake-oidc:latest`) y se levanta como un servicio más en `docker-compose`, igual que Postgres o cualquier otra dependencia de infra. Que quedó bien aplicado se nota en que, tanto en local como en tests de aceptación, se obtiene un token real firmado y validado contra JWKS de verdad — no hay ningún camino donde la API "confíe" en la autenticación sin comprobarla.
...
- **Mockear el token a mano (firmarlo tú mismo con una clave de test)**: funciona para tests, pero no da una pantalla de login real para desarrollo, y dos implementaciones (una para local, otra para tests) tienden a divergir. Es la variante usada, junto a `fake-oidc`, en un repo propietario (sin enlace): una implementación propia en el host de fakes de esa API, usada solo por tests de aceptación para mintar un token de forma no interactiva, que convive con `fake-oidc` — este último sigue siendo el único de los dos pensado para que una persona inicie sesión de verdad desde un navegador.

## Ejemplos

Adoptado en cuatro repos: Ne2Studio, [El Baúl](https://github.com/ne2-studio/el-baul), [CashClarity](https://github.com/ne2-studio/cashclarity) y un repo propietario (sin enlace). Mismo servicio, misma imagen, misma configuración por `OIDC_CLIENTS`/`OIDC_USERS`, con clientes y usuarios distintos según lo que cada API necesite.
