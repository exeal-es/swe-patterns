...
- **El análisis es una aproximación, no una compilación real**, y el propio código lo documenta: un escaneo por regex basta mientras el estilo de código sea consistente (un namespace por fichero, imports explícitos), pero infracontaría aristas si el proyecto adoptase global usings o referencias por nombre completo in place. Es una elección consciente de mantenerlo simple y sin dependencia de un compilador real, a cambio de ese límite conocido.
- **Snapshot y documentación se generan del mismo comando** para que no puedan divergir — si se regenerase el doc a mano por separado del snapshot, con el tiempo dejarían de coincidir y el doc mentiría sobre la arquitectura real.
- **El coste de aprobar es deliberadamente alto**: no hay un flag de "aceptar automáticamente", `./scripts/dsm approve` exige haber corrido el test, visto el fallo y revisado el diff primero.
